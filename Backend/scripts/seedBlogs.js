...
const seedBlogs = async () => {
    try {
        ...
        for (let i = 0; i < 35; i++) {
            ...
            const randomLikes = Math.floor(Math.random() * (300 - 10 + 1)) + 10;

            const randomFile = files[Math.floor(Math.random() * files.length)];
            const fileUrl = `${API}/public/uploads/${randomFile}`;

            // Convert your content array into a single HTML string
            const blogContentArray = [
                `This is some sample content for blog #${i + 1}. Lorem ipsum dolor sit amet, consectetur adipiscing elit...`,
                `Further content for blog Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.
        But what if we need to monitor something like user behavior on a website? Imagine running a web store where you sell products...`,
                `<img src="${fileUrl}" alt="Blog image ${i + 1}" />`, // Embed the image using HTML img tag
            ];

            const blogContentString = blogContentArray.join('\n\n');

            const blog = new Blog({
                title: `Sample Blog Title ${i + 1}`,
                content: blogContentString,
                topic: new mongoose.Types.ObjectId(randomTopic),
                author: new mongoose.Types.ObjectId(randomUser),
                views: randomViews,
                claps: randomLikes,
            });
